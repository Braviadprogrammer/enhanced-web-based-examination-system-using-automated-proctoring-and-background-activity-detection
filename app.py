...
from flask_wtf.file import FileField, FileRequired, FileAllowed #Correct Import added.
...
#import background_activities_detection_model as model

#import audio_processing as ap
#from audio_processing import audio_processing as ap
#import background_activities_detection_model as model
#import threading
#import time
...
import eventlet
import audio_processing.audio_utils as audio_utils
...
app = Flask(__name__)
socketio = SocketIO(app, async_mode='eventlet') # Initialize socketio here!
detection_active = False
warning_message = None
#audio_model = model.load_model() #load the model

# MySQL Configuration
app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'Bravia'
app.config['MYSQL_PORT'] = 3306
app.config['MYSQL_PASSWORD'] = os.getenv('MYSQL_PASSWORD')  # Use environment variables in production
...
app.config['MAIL_PASSWORD'] = os.getenv('MAIL_PASSWORD')  # Store this securely, ideally in environment variables
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USE_SSL'] = False

# Session Configuration
app.config['SECRET_KEY'] = 'BraviaProctureProject'  # Ensure this is a strong and unique key
app.config['SESSION_TYPE'] = 'filesystem'  # Consider 'redis' for production environments
app.config['SESSION_PERMANENT'] = False  # Adjust based on your session management needs
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=30)
app.config['SESSION_COOKIE_SAMESITE'] = "None"  # For cross-site cookie usage
app.config['SESSION_COOKIE_SECURE'] = True  # Only send cookies over HTTPS

# CORS Configuration
cors = CORS(app)
app.config['CORS_HEADERS'] = 'Content-Type'  # Modify as needed

# Stripe Configuration
stripe_keys = {
    "secret_key": "dummy",  # Replace with your actual secret key
    "publishable_key": "dummy",  # Replace with your actual publishable key
}
...
# Email Sender
sender = 'braviadprogrammer@gmail.com'
...
# Optional: Use environment variables for sensitive data in production
# Consider using python-dotenv for loading environment variables

#Load models
emotion_model = tf.keras.models.load_model('models/emotion_model.h5')
noise_model = tf.keras.models.load_model('models/noise_model.h5')
noise_filtering_model = tf.keras.models.load_model('models/noise_filtering_model.h5')
speech_model = tf.keras.models.load_model('models/speech_model.h5')
...
def video_feed():
	if request.method == "POST":
		...
		results = cur.execute('INSERT INTO proctoring_log (email, name, test_id, voice_db, img_log, user_movements_updown, user_movements_lr, user_movements_eyes, phone_detection, person_status, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
			(session.get('email', ''), dict(session)['name'], testid, voice_db, jpg_as_text, user_move1, user_move2, eye_movements, mob_status, person_status,session.get('uid', '')))
...
def window_event():
	if request.method == "POST":
		...
		cur = mysql.connection.cursor()
		results = cur.execute('INSERT INTO window_estimation_log (email, test_id, name, window_event, uid) values(%s,%s,%s,%s,%s)', (session.get('email', ''), testid, dict(session)['name'], 1, session.get('uid', '')))
		mysql.connection.commit()
		cur.close()
		if(results > 0):
...
def register():
    if request.method == 'POST':
        # Get form data and check if they are provided
        ...
        if not name or not email or not password or not user_type or not imgdata:
            # If any form data is missing, provide feedback
            flash('All fields are required. Please fill them out.', 'error')
        ...
        sesOTP = generateOTP()  # Assuming generateOTP() is already defined elsewhere
        ...
        try:
            ...
            return redirect(url_for('verifyEmail'))  # Redirect to OTP verification page
            
        except Exception as e:
            # Handle errors during email sending
            flash('There was an error sending the OTP. Please try again later.', 'error')
...
def verifyEmail():
    if request.method == 'POST':
        ...
        if not mOTP or not dbName or not dbEmail or not dbPassword:
            flash("Session expired or invalid data, please register again.")
            return redirect(url_for('register'))
        
        # Check if OTP was provided
        if not theOTP:
            return render_template('verifyEmail.html', error="OTP is missing.")
        ...
        if theOTP == mOTP:
            ...
            if ar > 0:
                ...
                session.clear()  # Clear session after successful registration
                return redirect(url_for('login'))
            else:
                flash("An error occurred while processing your registration. Please try again.")
                return redirect(url_for('verifyEmail'))

            cur.close()  # Always close the cursor after execution
        else:
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			...
			cur.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc, proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
				(session.get('email', ''), test_id, "subjective", start_date_time, end_date_time, duration, 0, password, subject, topic, 0, 0, proctor_type, session['uid']))
			mysql.connection.commit()
...
def create_test():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			...
			proctor_type = form.proctor_type.data
			cur.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc,proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
				(session.get('email', ''), test_id, "objective", start_date_time, end_date_time, duration, 1, password, subject, topic, neg_mark, calc, proctor_type, session['uid']))
			mysql.connection.commit()
			cur.execute('UPDATE users SET examcredits = examcredits-1 where email = %s and uid = %s', (session['email'],session['uid']))
			mysql.connection.commit()
			cur.close()
...
def create_test_pqa():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			...
			proctor_type = form.proctor_type.data
			cur.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc, proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
				(session.get('email', ''), test_id, "practical", start_date_time, end_date_time, duration, 0, password, subject, topic, 0, 0, proctor_type, session['uid']))
			mysql.connection.commit()
			cur.execute('UPDATE users SET examcredits = examcredits-1 where email = %s and uid = %s', (session['email'],session['uid']))
			mysql.connection.commit()
			cur.close()
...
@app.route("/background_activities_detection") # Changed route here
def background_activities_detection():
    return render_template("background_activities_detection.html")

@socketio.on('start_detection')
def handle_start_detection():
    try:
        audio = audio_utils.capture_audio()
        rms = audio_utils.calculate_rms(audio)

        threshold = 0.00005  # Very low threshold

        noise_detected = audio_utils.is_noise_detected(rms, threshold)

        rms_float = float(rms)
        noise_detected_bool = bool(noise_detected)

        logging.info(f"RMS: {rms_float}, Threshold: {threshold}, Noise Detected: {noise_detected_bool}")

        emit('detection_result', {'rms': rms_float, 'noise_detected': noise_detected_bool})
    except Exception as e:
...
def background_detection():
    global detection_active, warning_message
    try:
        while detection_active:
            audio_data = ap.capture_audio()

            if ap.detect_voice_activity(audio_data):
                warning_message = "Voice activity detected."
                print("Voice activity detected")
            elif not ap.detect_silence(audio_data):
                warning_message = "Background noise detected."
...
def real_time_background_activities_detection():
    return render_template('real_time_background_activities_detection.html')

@app.route('/toggle_detection', methods=['POST'])
def toggle_detection():
    global detection_active, warning_message, detection_thread
    detection_active = not detection_active
    warning_message = None
    if detection_active:
        detection_thread = threading.Thread(target=background_detection)
...
def get_warning():
    ...
    return jsonify({'warning': warning_message})


def predict_audio(audio_data, sample_rate):
    """Predicts emotion, noise, and speech labels from audio data."""
    mfccs = extract_features(audio_data, max_pad_len=174)
    if mfccs is None:
        return -1, -1, -1, -1  # Indicate an error if feature extraction fails

    mfccs = mfccs[np.newaxis, ..., np.newaxis]
    emotion_prediction = emotion_model.predict(mfccs)
    noise_prediction = noise_model.predict(mfccs)
    noise_filtering_prediction = noise_filtering_model.predict(mfccs)
    speech_prediction = speech_model.predict(mfccs)

    emotion_label = np.argmax(emotion_prediction)
    noise_label = np.argmax(noise_prediction)
    noise_filtering_label = np.argmax(noise_filtering_prediction)
    speech_label = np.argmax(speech_prediction)

    return emotion_label, noise_label, noise_filtering_label, speech_label
...
def predict():
    ...
    try:
        ...
        emotion, noise, noise_filtering, speech = predict_audio(audio_data, sample_rate)

        # Logic to determine if exam should be stopped.
        exam_stop = False
        reason = ""
        ...
        if noise_filtering != 0:
            exam_stop = True
            reason += "Noise filtering detected. "

        # Add emotion logic here, if needed.

        response = {
            'emotion': int(emotion),
            'noise': int(noise),
            'noise_filtering': int(noise_filtering),
            'speech': int(speech),
            'exam_stop': exam_stop,
            'reason': reason,
        }
    ...
    socketio.run(app, host="0.0.0.0", debug=True) # add host="0.0.0.0"
